/* Vanilla JS port of ReactBits Waves */
(function () {
  ...
  function init() {
    var container = document.getElementById('services-waves');
    if (!container) return;
    ...
    function setLines() {
      lines = [];
      var oWidth = bounds.width + 200, oHeight = bounds.height + 30;
      var totalLines = Math.ceil(oWidth / config.xGap);
      var totalPoints = Math.ceil(oHeight / config.yGap);
      var xStart = (bounds.width - config.xGap * totalLines) / 2;
      var yStart = (bounds.height - config.yGap * totalPoints) / 2;
    ...
    function movePoints(time) {
      lines.forEach(function(pts) {
        pts.forEach(function(p) {
          var move = noise.perlin2((p.x + time * config.waveSpeedX) * 0.002, (p.y + time * config.waveSpeedY) * 0.0015) * 12;
